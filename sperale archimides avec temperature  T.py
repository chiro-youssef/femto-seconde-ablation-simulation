...
v = 2e6  # Vitesse de balayage (µm/s)
...
kappa = 0.2e-5  # Diffusivité thermique (m²/s)
...
a = (40e-6) / (5 * np.pi)  # Espacement pour r_max ≈ 40 µm après 5 tours
theta_max = 8 * np.pi  # 5 tours
...
im = ax.imshow(np.zeros_like(X), extent=[-100, 100, -100, 100], cmap='jet', vmin=0, vmax=1500)
...
ani.save('ablation_spiral_thermal.gif', writer='pillow', fps=10)
plt.show()
