...
kappa = 5e-6  # Diffusivité thermique (m²/s)
...
a = (30e-6) / (5 * np.pi)  # Espacement pour r_max ≈ 40 µm après 5 tours
...
im = ax.imshow(np.zeros_like(X), extent=[-80, 80, -80, 80], cmap='jet', vmin=0, vmax=1000)
...
ani.save('ablation_inward_archimedean_spiral_thermal.gif', writer='pillow', fps=10)
plt.show()
...
ax_static.set_title('Profondeur d\'ablation finale (Spirale d\'Archimède Inward)')
...
ax_3d.set_title('Profondeur d\'ablation finale (3D, Spirale d\'Archimède Inward)')
